...
//Update de cadeiras
app.patch('/cadeira/:id', async (req, res) => {
    ...
    try {
        ...
        const updateCadeira = await Cadeira.updateOne({ _id: id }, cadeira)
        if (updateCadeira.matchedCount === 0) {

            res.status(422).json({ message: 'Cadeira não encontrada' })
            return

        }
        res.status(200).json(cadeira)
...
//Deletar cadeiras
app.delete('cadeira/:id', async (req, res) => {

    const id = req.params.id

    const cadeira = await Cadeira.findOne({ _id: id });
    ...
    try {
        ...
        await Cadeira.deleteOne({ _id: id })
        res.status(200).json({ message: 'Cadeira removida com sucesso!' })
